...
inline int Game3World::getRandomRoom(BOOL chalice, int empty)
{
	int loopTimes = 0;
	int randNum = RandomGenerator::getRandomNumber(GAME2_NUMBER_OF_ROOMS);
	do
	{
		...
		loopTimes++;
		randNum = RandomGenerator::getRandomNumber(GAME2_NUMBER_OF_ROOMS);
	}
	...
	for (int i=0;i<GAME2_NUMBER_OF_ROOMS;i++)
	{
	...
	Logger::error("A room was still not found, so therefore there is a proble with the world, since there are more objects to be placed randomly than there are rooms");
	assert(FALSE);
	return -1;
}

Game3World::Game3World(BOOL difficult,BOOL extraItems) : Game2World(difficult,extraItems) {}

inline BOOL Game3World::isRoomSuitable(Room *room, int number, BOOL chalice, int empty)
{
...
void Game3World::setObjectsPositionsAndRooms()
{
	for (int i=0;i<GAME2_NUMBER_OF_ROOMS;i++) { usedRooms[i] = 0; }
